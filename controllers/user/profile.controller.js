...
const getUserById = async (req, res, next) => {
	const { id } = req.params;
	try {
		const user = await userModel.findById(id).populate("department");
		return res.status(StatusCodes.OK).json({
			success: true,
...
const createUser = async (req, res, next) => {
	try {
		let { username, email, photo, role, department } = req.body;
		...
		if (req.user.role !== ROLES.SUPER_ADMIN && department?.toString() !== req.user?.department?._id?.toString()) {
			return res.status(StatusCodes.BAD_REQUEST).json({
				success: false,
		...
		const emailResponse = await sendEmail(
			...
			getImportRegistrationHTML(username, email, randomPassword)
		);
...
const updateProfile = async (req, res, next) => {
	try {
		...
		const { username, photo, importantDates, syncWithGoogle, donotDisturbState, notificationExpiry, activeSemester } =
			req.body;
...
const updateUser = async (req, res, next) => {
	try {
		...
		const { department, role, permissions, reportsTo } = req.body;

		if (req.user?.role === ROLES.SUPER_ADMIN && reportsTo) {
			await userModel.findByIdAndUpdate(user._id, { reportsTo });
		}
...
const deleteUser = async (req, res, next) => {
	try {
		...
		if (
			...
			user.department.toString() !== req.user?.department?._id?.toString()
		) {
			return res.status(StatusCodes.UNAUTHORIZED).json({
				success: false,
				message: "Unauthorized to delete this user",
