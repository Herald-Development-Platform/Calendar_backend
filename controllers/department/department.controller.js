...
const createDepartment = async (req, res, next) => {
  try {
    ...
    if (req.body.admins && req.body.admins.length > 0) {
      const admins = await models.userModel
        .find({ _id: { $in: req.body.admins } })
        .populate("department");
...
const updateDepartment = async (req, res, next) => {
  try {
    ...
    const updatedDepartment = await models.departmentModel.findByIdAndUpdate(
      department._id,
      req.body,
      { new: true }
    );
...
const deleteDepartment = async (req, res, next) => {
  try {
    ...
    await models.userModel.updateMany({ department: department._id }, { department: null });
    await models.userModel.updateMany(
