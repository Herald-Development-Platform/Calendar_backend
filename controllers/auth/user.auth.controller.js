...
const userRegister = async (req, res, next) => {
  try {
    ...
    if (!process.env.ALLOW_INVALID_EMAILS) {
      if (COLLEGEID_REGEX.test(email)) {
        return res.status(StatusCodes.BAD_REQUEST).json({
          success: false,
          message:
            "Students dont have access to this system. Please contact admin for more details.",
        });
      }
    ...
    const response = await sendEmail(
      ...
      getRegistrationHTML(username, OTP, email)
    );
...
const forgetPassword = async (req, res, next) => {
  try {
    ...
    const response = await sendEmail(
      ...
      [],
      "Password Reset",
      getForgetPasswordHTML(user.username, OTP)
    );
    if (!response.success) {
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
        success: false,
