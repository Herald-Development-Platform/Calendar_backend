  ...
  try {
    ...
    if (req.user.role !== ROLES.SUPER_ADMIN) {
      ...
      if (departments && departments.length > 0) {
        departmentsToFilter = await Promise.all(
        ...
        departmentsToFilter = (await models.departmentModel.find({})).map(department =>
    ...
    createEvents(eventDetailsArray, (error, value) => {
      ...
      return res
        ...
        .json({ success: true, message: "Successfully exported to ICS string!", data: value });
    });
  } catch (error) {
    return next(error);
  }
