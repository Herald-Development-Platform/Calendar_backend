...
const sendOngoingEventsNotification = async () => {
  try {
    let nonRecurringEvents = await models.eventModel
      .find({
    ...
    let recurringEvents = await models.eventModel
      .find({
    ...
    recurringEvents.forEach(event => {
      ...
      if (
        event.notifiedDates.length > 0 &&
        event.notifiedDates[event.notifiedDates.length - 1] > new Date() - 60 * 60 * 1000
      ) {
        return;
      }
    ...
    let allEvents = [...nonRecurringEvents, ...recurringEventsWithOccurrences];

    for (let event of allEvents) {
      const differenceString = timeDifference(event.start);
      let emailUsers = [];
      emailUsers = emailUsers.concat(event.involvedUsers);
      emailUsers = emailUsers.concat(
        await models.userModel.find({ department: { $in: event.departments?.map(d => d._id) } })
      );
      if (event.createdBy) {
