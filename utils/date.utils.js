function timeDifference(input) {
  ...
  if (diffMs < 0) {
    if (diffYears !== 0)
      return `${Math.abs(diffYears)} ${pluralize(Math.abs(diffYears), "year")} ago`;
    if (diffMonths !== 0)
      return `${Math.abs(diffMonths)} ${pluralize(Math.abs(diffMonths), "month")} ago`;
    if (diffDays !== 0) return `${Math.abs(diffDays)} ${pluralize(Math.abs(diffDays), "day")} ago`;
    if (diffHours !== 0)
      return `${Math.abs(diffHours)} ${pluralize(Math.abs(diffHours), "hour")} ago`;
    if (diffMinutes !== 0)
      return `${Math.abs(diffMinutes)} ${pluralize(Math.abs(diffMinutes), "minute")} ago`;
